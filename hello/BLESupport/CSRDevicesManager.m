...
- (void)associateDeviceFromCSRDeviceManager:(NSData *)deviceHash authorisationCode:(NSData *)authCode deviceID:(NSNumber *)deviceId
{    
...
- (void) setPowerState:(NSNumber *)deviceId state:(NSNumber *)state {
    ...
    [[PowerModelApi sharedInstance] setPowerState:deviceId
                                          ...
                                          success:^(NSNumber * _Nullable deviceId, NSNumber * _Nullable state) {
                                              NSLog(@"state success :%@", state);
                                          } failure:^(NSError * _Nullable error) {
                                              NSLog(@"error %@", error);
                                          }];
 }

- (void) sendData:(NSNumber *)deviceId data:(NSData *)data {
    [[DataModelApi sharedInstance]sendData:deviceId data:data success:nil failure:nil];
...
- (void)didSendData:(NSNumber *)deviceId
    ...
    NSLog(@"data is %@ and data is %@ and device id %@", str, data, deviceId);
}
...
- (void)didReceiveStreamDataEnd:(NSNumber *)deviceId
                   streamNumber:(NSNumber *)streamNumber {
    NSLog(@"data is %@ and data is %@", deviceId, streamNumber);
}
...
- (void)didReceiveBlockData:(NSNumber *)destinationDeviceId
             sourceDeviceId:(NSNumber *)sourceDeviceId
                       data:(NSData *)data {
    NSString *str = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    NSLog(@"data is %@ and data is %@ and device id %@", str, data, sourceDeviceId);
    [[ServiceUtility getObj] processRecievedDataFromMeshWithDeviceId:sourceDeviceId data:data];
}
