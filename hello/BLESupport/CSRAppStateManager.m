...
- (void)createDefaultPlace
{
    ...
    if ([places count] <= 0) {
        ...
        defaultPlace.passPhrase = [[ServiceUtility getObj] generatePhaprase];
    ...
    if ([placeID isKindOfClass:[NSManagedObjectID class]]) {
        ...
        [placesArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if ([[place objectID] isEqual:placeID]) {
                ...
                if (self.selectedPlace.passPhrase) {
                    [[CSRDevicesManager sharedInstance] setNetworkPassPhrase:self.selectedPlace.passPhrase];
                } else {
                    [[CSRDevicesManager sharedInstance] setNetworkKey:self.selectedPlace.networkKey];
                }
                self.selectedPlace.settings.cloudMeshID = [CSRMeshUserManager sharedInstance].meshId;
