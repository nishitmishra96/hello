...
class FileAccessor {
    ...
    /**
     *  Copies file from store's relative path to Absolute path
     */
    ...
    /**
     * Creates a directory with the given relative path, including any intermediate directories.
     * Does nothing if the directory already exists.
     */
    func createDirectoryAtRelativepath(_ path:String?) throws {
        var absolutePath = rootPath
        if let relativeDir = path {
            absolutePath = URL(fileURLWithPath: absolutePath).appendingPathComponent(relativeDir).path
        }
        try createDir(absolutePath)
    }
    ...
    /**
     * Creates a File with the given relative path
     */
    func createNewFileAtPathIfNotAvailable(filename:String) throws {
